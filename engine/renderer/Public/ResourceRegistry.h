...
namespace Arche {
    namespace Render {
        ...
        class ResourceRegistry {
          public:
            // Mesh
            void registerMesh(std::shared_ptr<Mesh> mesh) { if (mesh) m_meshes[mesh->getName()] = std::move(mesh); }
            ...
            // Material
            void registerMaterial(std::shared_ptr<Material> mat) { if (mat) m_materials[mat->getName()] = std::move(mat); }
            ...
            // Shader
            void registerShader(std::shared_ptr<Shader> sh) { if (sh) m_shaders[sh->getName()] = std::move(sh); }
