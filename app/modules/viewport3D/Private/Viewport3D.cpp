    ...
    namespace GUI {
        ...
        void Viewport3DPanel::Draw() {
            ...
            for (const auto &body : view.bodies) {
                ...
                const auto bodyPosition{body.transform.getPosition()};
            ...
            if (mouseClicked && bestBodyId) {
                ...
                auto selectedIt = std::find_if(view.bodies.begin(), view.bodies.end(),
                                               [&](const auto &b) { return b.id == *localSelectedBodyId; });

                if (selectedIt != view.bodies.end()) {

                    auto p = selectedIt->transform.getPosition();
                    editState =
                        EditState{p.x(), p.y(), selectedIt->mass, selectedIt->transform.getScale().x(), selectedIt->id};

                    std::ostringstream logOss;
                    logOss << "Clicked on particle id " << selectedIt->id << " at position (" << p.x() << ", " << p.y()
                           << ")";
...

            {
                ...
                if (hoveringCanvas && allowControl) {
                    ...
                    if (ImGui::IsMouseDown(ImGuiMouseButton_Right)) {
                        cameraController->rotate(-mouseDelta.x, -mouseDelta.y);
                        cameraController->updateCamera();
                    }

                    if (ImGui::IsMouseDown(ImGuiMouseButton_Middle)) {
                        cameraController->pan(-mouseDelta.x, mouseDelta.y);
                        cameraController->updateCamera();
                    }
                    ...
                    float baseSpeed{cameraController->getDistance() > 0.0f ? cameraController->getDistance() * 0.5f
                                                                           : 1.0f};
                    ...
                    if (forward != 0.0f || right != 0.0f || up != 0.0f) {
                        ...
                        if (context && context->renderer() && world) {
                            context->renderer()->render(world->view().bodies);
                        }
                    }
                }
            ...
            if (ImGui::BeginPopup("ViewportContextMenu")) {
                ...
                if (ImGui::MenuItem("Add Particle Here")) {
                    glm::dvec3 rayDirection{camera->screenToWorldRay(mousePos.x - canvasP0.x, mousePos.y - canvasP0.y,
                                                                     canvasSize.x, canvasSize.y)};

                    const double spawnDistance{20.0};
                    glm::dvec3 cameraPos = camera->GetPosition();
                    glm::dvec3 spawnWorldPos{cameraPos + rayDirection * spawnDistance};

                    Arche::Math::SpatialTransform transform;
                    transform.setPosition(Arche::Math::Vector3D(spawnWorldPos.x, spawnWorldPos.y, spawnWorldPos.z));
                    transform.setScale(Arche::Math::Vector3D(10.0, 10.0, 10.0));
                    world->createParticle(transform, 1.0);
                }

                ImGui::EndPopup();
            }

            auto selectedIt = selectedBodyId ? std::find_if(view.bodies.begin(), view.bodies.end(),
                                                            [&](const auto &b) { return b.id == *selectedBodyId; })
                                             : view.bodies.end();
            ...
            if (ImGui::BeginPopupModal("EditObjectPopup", nullptr, ImGuiWindowFlags_AlwaysAutoResize)) {
                if (selectedIt != view.bodies.end() && editState && editState->editingId == selectedIt->id) {
                    ...
                    if (ImGui::Button("Save")) {
                        ...
                        world->setObjectPosition(selectedIt->id,
                                                 Arche::Math::Vector3D(editState->posX, editState->posY,
                                                                       selectedIt->transform.getPosition().z()));
                        world->setObjectMass(selectedIt->id, static_cast<float>(editState->mass));
                        world->setObjectScale(selectedIt->id, static_cast<float>(editState->scale));
                        ImGui::CloseCurrentPopup();
                        editState.reset();
                        // Clear selection so the modal doesn't reopen next frame and block UI
                    ...
                    if (ImGui::Button("Cancel")) {
                        ImGui::CloseCurrentPopup();
                        editState.reset();
                        // Also clear selection on cancel
                        selectedBodyId.reset();
                    }
                } else {
                    ImGui::CloseCurrentPopup();
            ...
            // --- Camera overlay drawn last so it appears on top of renderer image ---
            {
                ...
                oss.str("");
                oss.clear();
                oss << "Pos: (" << std::fixed << std::setprecision(2) << camPos.x << ", " << camPos.y << ", "
                    << camPos.z << ")";
                ...
                dl->AddLine(ImVec2(boxMin.x + padding.x * 0.5f, sepY + 4.0f * uiScale3),
                            ImVec2(boxMax.x - padding.x * 0.5f, sepY + 4.0f * uiScale3),
                            IM_COL32(120, 120, 130, 80), 1.0f * uiScale3);
